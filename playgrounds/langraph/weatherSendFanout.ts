...
const USER_INPUT = "帮我看一下上海、北京、杭州今天的天气，再顺手告诉我深圳是不是更热。";
...
function buildGraph() {
  async function parseCitiesNode(state: GraphState): Promise<GraphUpdate> {
    printStep("parseCities");
    const requestedCities = parseCities(state.userInput);

    console.log("input:", state.userInput);
    console.log("requestedCities:", requestedCities.join(", ") || "<none>");
  ...
  async function summarizeWeatherNode(state: GraphState): Promise<GraphUpdate> {
    ...
    const summary = [
      ...
      hottestCity ? `最热的是 ${hottestCity.city}，当前 ${hottestCity.temperatureC}°C。` : "没有可汇总的天气结果。",
    ].join("\n");
  ...
  return new StateGraph(graphState)
    .addNode("parseCities", parseCitiesNode)
    ...
    // 根据 parseCities 的结果，动态路由到多个 getWeather 节点(并行)，或者直接路由到 noCities 节点
    .addConditionalEdges("parseCities", routeToWeatherLookups)
...
function routeToWeatherLookups(state: GraphState): Send<"getWeather", { city: string }>[] | "noCities" {
  if (state.requestedCities.length === 0) {
    return "noCities";
  }

  return state.requestedCities.map((city) => new Send("getWeather", { city }));
}
...
main().catch((error) => {
  console.error("weather-send-fanout failed:", error instanceof Error ? error.message : error);
  process.exitCode = 1;
});